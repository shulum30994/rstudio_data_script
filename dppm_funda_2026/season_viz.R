...
##### All Farmers #####
all_farmers_season<-season_start_end %>%
  ggplot()+
  geom_segment(
    aes(
      x=start, xend=end,
  ...
  labs(title="Waktu Musim Tanam Responden & Anjuran Kementerian Pertanian 2026 di Area Survey",
...
##### Jember #####
jember<-season_start_end %>%
  filter(region=="JEMBER") %>%
  ggplot()+
  geom_segment(
    aes(
      x=start, xend=end,
...
kudus<-season_start_end %>%
  ...
  xlab("Waktu")+
  ylab("Petani/Responden")+
  labs(title="Waktu Musim Tanam Responden & Anjuran Kementerian Pertanian 2026 di Area Survey Kudus",
subtitle = "Hasil survey telah disesuaikan dengan ASEP 2026",
color="Keterangan :",
caption="Sumber : Hasil Survey")+
  theme_linedraw()+
